...
def generatePalindromes(s: str) -> List[str]:
    ...
    res = []
    visited = [0] * len(half)
    self.helper(visited, half ,"", res,0)
    return res


def helper(visited, half, path, res,idx):
    if len(path) == len(half):
        res.append(path + self.mid + path[::-1])
        return
    
    for i in range(idx,len(half)):
        if i > 0 and visited[i-1] == 0 and half[i] == half[i-1]: continue
        if visited[i] == 0:
            visited[i] = 1
            self.helper(visited, half, path + half[i], res,idx)
            visited[i] = 0
