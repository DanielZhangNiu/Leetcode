...
def dfs(head, ad_list,tinput_list):
    cluster = []
    pointer = [head]
    while len(pointer) > 0:
        circle1 = pointer.pop(0)
        if cluster.count(circle1) >= tinput_list.count(circle1):
            continue
        ...
        cluster = cluster + [circle1]  # add circle to cluster
    #print(cluster)
    return cluster
...
def circle_cluster(input_list, overlapped):
    ...
    while len(input_list) > 0:
        clusters = dfs(input_list[0], overlapped, input_list)
